                    ...
                    ->schema([
                        Radio::make('acc_level')
                            ...
                            ->afterStateUpdated(function (Set $set,$state,Get $get){
                                ...
                                $set('theSon', null);
                                if ($state==1) {
                                    $num=Account::where('acc_level',1)->max('num')+1;
                                    $set('num',$num);
                                    $set('id',strval($num));
                                }
                        ...
                        Hidden::make('is_active')
                            ->afterStateHydrated(function (Set $set,Get $get): void {
                                ...
                                if ($last){
                                    ...
                                    if ($last->acc_level->value==1) {
                                      $num=Account::where('acc_level',1)->max('num')+1;
                                        $set('num',$num);
                                        $set('id',strval($num));
                                    }
                                    if ($last->acc_level->value==2) {
                                        $grand=$last->grand_id;
                                        $set('grand_id',$grand);
                                        $set('theGrand',$grand);
                                        $num=Account::where('grand_id',$grand)->max('num')+1;
                                        $set('num',$num);
                                        $set('id',$grand.'-'.strval($num));
                                    }
                                    if ($last->acc_level->value==3) {
                                        $grand=$last->grand_id;
                                        $father=$last->father_id;
                                        $set('grand_id',$grand);
                                        $set('theGrand',$grand);
                                        $set('theFather',$father);
                                        $set('father_id',$father);
                                        $num=Account::where('father_id',$father)->max('num')+1;
                                        $set('num',$num);
                                        $set('id',$father.'-'.strval($num));
                                    }
                                    if ($last->acc_level->value==4) {
                                        $grand=$last->grand_id;
                                        $father=$last->father_id;
                                        $son=$last->son_id;
                                        $set('grand_id',$grand);
                                        ...
                                        $set('son_id',$son);
                                        $set('theSon',$son);
                                        $num=Account::where('son_id',$son)->max('num')+1;
                                        $set('num',$num);
                                        $set('id',$son.'-'.strval($num));
                                    }
                                }
                            })
                            ->default(1) ,
